...
const PREVIOUS_DEPLOYMENT_ID = "dpl_2Wd59wMMiPm7NixbWamjfMqdjR9a";
const DEPLOYMENT_ID = "dpl_9L1STHexMDa1pELygfbbubgU7eP7";
const CRAWL_ID = "cmsw4uwtp0opmn101t2hmmsp2";
const CHANGED_AT = "2026-08-16T18:21:23.000Z";
const CHECKED_AT = "2026-08-16T18:23:34.211Z";
const DAY7_DUE_AT = "2026-08-23T18:21:23.000Z";
const DAY28_DUE_AT = "2026-09-13T18:21:23.000Z";
const DAY56_DUE_AT = "2026-10-11T18:21:23.000Z";
...
function payloadFor(current: Parameters<typeof pageReviewToApi>[0]) {
  ...
  const directDecisionSummary =
    "Positive river reports now lead with ‘Good fishing’ and ‘Yes. Fishing looks good’ when verified flow and weather support the trip; a missing official live water-temperature reading remains a separate warm-weather precaution instead of changing the answer to ‘Maybe’.";
  const photographSummary =
    "The Lochsa hero now uses a genuine U.S. Forest Service Northern Region photograph of the Lochsa River in Clearwater National Forest with CC BY 2.0 attribution instead of generated artwork.";
  ...
  return patchPageReviewSchema.parse({
    ...
    manualReview: {
      ...
      notes:
        `${review.manualReview.notes ?? ""}\n\nProduction release ${DEPLOYMENT_ID} made the Lochsa trip decision direct: Good fishing / Yes. Fishing looks good, based on verified flow and weather, with the unavailable official live water temperature explained separately. The release also replaced generated art with a genuine U.S. Forest Service Northern Region Lochsa photograph under CC BY 2.0. Vercel marked the production deployment READY and aliased it to bluestreamfly.com. Post-deploy sitemap-only crawl ${CRAWL_ID} fetched 669/669 canonicals; Lochsa returned 200, remained indexable with its exact canonical, and kept content score 100.`.trim(),
    },
    manualChatState: "monitoring",
    userDecisionReference: review.userDecisionReference,
    changeNote:
      `Record the final direct fishing decision, genuine Lochsa photograph, production deployment ${DEPLOYMENT_ID}, canonical crawl ${CRAWL_ID}, and reset Day-7/28/56 monitoring dates from this release.`,
  });
}
